...
`raw-research/` is reserved for batch material the operator pre-organizes; do not auto-route there.

## Ambiguity rule

When inferring `source_kind` is genuinely ambiguous (e.g., a long Reddit comment — clipping or article?), pick the closer fit and surface that decision: `Reading this as a clipping, not an article — flag if you'd rather route to articles/.` Don't ask the operator to pick from the menu; pick and surface.
