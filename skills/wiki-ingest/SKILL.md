---
name: wiki-ingest
description: Use when the user asks to ingest a source into his Obsidian vault at ~/Documents/brain/, invokes /wiki-ingest, pastes source-shaped content into chat (a URL with extract, a multi-paragraph quote, a tweet body, a Perplexity output, a GitHub repo URL, an article excerpt), OR when Claude's own web research (WebSearch/WebFetch) surfaces a substantive, reusable source worth keeping.
...
Process a new source into the user's wiki at `~/Documents/brain/`. Implements the ingest workflow defined in `~/Documents/brain/SCHEMA.md`. Three entry points: explicit (`/wiki-ingest <path>` or "ingest this"), **auto-trigger on chat-paste** (the user pastes source-shaped content), and **auto-trigger on web research** (Claude's own WebSearch/WebFetch surfaces a file-worthy source).

## Auto-trigger on chat-paste

Fires when the user pastes source-shaped content into chat without explicit invocation. **Detection is qualitative -- no token-count gate, no regex.** Claude judges whether the pasted content looks like a source (see `llm-internal-heuristics-prefer-judgment` memory pattern).

**Source-shaped (judge yes):** standalone URL where the user's framing signals filing not querying; multi-paragraph quote or extract; Perplexity Pro output; AI session handoff to save.
...
Fires when **Claude's own** WebSearch / WebFetch during a task surfaces a substantive, reusable source worth keeping. Bar: "would future-Claude or the user want this source again?" not "did a fetch succeed?"
...
Quick summary: tweet / article / clipping / repo / perplexity / paper / session / inbox -- each maps to `sources/<kind>/<YYYY-MM-DD>-<slug>.md`. `raw-research/` is reserved for batch material the user pre-organizes; never auto-route there. When `source_kind` is ambiguous, pick the closer fit and surface the decision (Rule 7).
...
Show the user a structured plan BEFORE writing anything. Full proposal template: `references/wiki-proposal-template.md`. Wait for the user's OK or revisions before proceeding.
...
On overlap: prompt the user to supersede (Y), merge (M), write in parallel (P), or skip (n). On no overlap: proceed silently to Step 4. Full protocol detail: `references/dedupe-protocol.md`.
...
11. **`description:` field is required.** Every source file must have it; `wiki-query` ranking depends on it.

## Anti-patterns

Full list: `references/anti-patterns.md`. Key violations: writing wiki pages before showing the user the plan; skipping Step 3.5 dedupe; asking the user which `source_kind` to use (infer and surface instead); writing a source without `description:`; auto-routing to `raw-research/`; capturing throwaway lookups.
...
- `~/.claude/rules/communication.md` § Surface decisions inline -- surface-and-proceed format
