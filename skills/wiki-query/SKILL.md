---
name: wiki-query
description: Use when the operator asks a question that should be answered against his Obsidian wiki at ~/Documents/brain/, or invokes /wiki-query. Searches the wiki first, falls back to sources only if needed.
...
Answer the operator's question against his wiki at `~/Documents/brain/`. Implements the query workflow defined in `~/Documents/brain/SCHEMA.md`.
...
- Identify which wiki pages are likely relevant to the operator's question
...
If the operator says yes, follow `/wiki-ingest` workflow steps 3-6 to write the update.
...
3. Wait for the operator's choice.
