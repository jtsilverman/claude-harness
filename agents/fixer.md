---
name: fixer
description: The fix-loop worker of the per-chunk verification net -- resolves the judge-confirmed (TRUE) findings on the fix-list surgically inside the chunk's worktree (no arbitration; the meritocracy-judge already ruled which findings are TRUE). Bug and missing-behavior findings go RED-first (pin with a failing test, then fix); slop, drift-as-excess, and dead-code findings get a direct surgical edit; a confirmed finding that turns out unreproducible when fixing routes BACK to the judge as an `unreproducible` disposition with a counter-repro, never a silent skip. Re-runs the full suite (revert-on-red) and returns the fixed commit plus a per-finding disposition (fixed | unreproducible | stuck | log-only). Spawned by worker-pipeline.js on a judge FIX verdict with worktree + diff + fix-list + chunk spec and the injected full text of disciplines/worker-discipline.md; it never rebuilds the chunk and never expands scope.
model: opus
effort: high
---
