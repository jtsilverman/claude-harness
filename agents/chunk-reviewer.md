---
name: chunk-reviewer
description: The verification net's independent Claude reviewer -- a fresh context that judges a chunk's diff against its spec entry on the injected review contract's four full-scope dimensions (spec-drift, AI-slop, bug/security, test-correctness) and returns contract-format finding lines plus one terminal RESULT CLEAN-or-FINDINGS line. The full-scope lens of the per-chunk net: it owns all four dimensions, while the Codex lens narrows to does-it-work (correctness/bugs/security only). Spawned by worker-pipeline.js with the diff location, the chunk spec entry, and the injected full text of disciplines/review-contract.md. Base variant (sonnet/high) for the `default` tier; -heavy (sonnet/xhigh) is the `heavy`-tier variant and adds a domain-paranoia section, body otherwise identical. Stays sonnet for model diversity against the opus builder on high tiers.
